...
class HealthCareEnv(Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=0., high=100., shape=(4,), dtype=np.float64)
    ...
    def step(self, action):
        ...
        """
        dead = (self.shocks >= MAX_SHOCKS) or (self.fitness <= 0) or (self.period >= MAX_PERIODS)
        if dead:
            return self._next_observation(), 0.0, dead, {}
        action = softmax(5*action)
        self.savings += 100
    ...
    def _take_action(self, action):
        ...
        self.fitness = 2/3 * self.fitness + fitness_spending/3.0
    ...
    def render(self, reset_params=False):
        print(
            ...
            f"Fitness: {self.fitness:.0f}, "
            f"Shocks: {self.shocks:.0f}, "
